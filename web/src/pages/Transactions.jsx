...
export function TransactionModal({ open, onClose, transaction }) {
    ...
    const [form, setForm] = useState(txData
        ? {
            ...
            toWalletId: txData.to_wallet_id || '',
            categoryId: txData.category_id || '',
            contactId: txData.contact_id || '',
            isDebt: !!txData.is_debt,
            date: isDuplicate ? format(new Date(), 'yyyy-MM-dd') : format(new Date(txData.date), 'yyyy-MM-dd')
        }
        : getEmptyForm());
    ...
    const isEdit = !!txData && !isDuplicate;

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            let saved;
            if (isEdit) { saved = await update.mutateAsync({ id: txData.id, ...form }); }
            else { saved = await create.mutateAsync(form); }
            if (isSplit && splits.length > 0) await saveSplits.mutateAsync({ id: saved.id, splits });
            toast(`Transaction ${isEdit ? 'updated' : 'created'}!`, 'success');
            onClose();
        } catch (err) { toast(err.response?.data?.error?.message || 'Error saving transaction', 'error'); }
    };

    const handleFormChange = (field, value) => setForm(p => ({ ...p, [field]: value }));

    return (
        <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Transaction' : 'New Transaction'} size="lg">
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <Field label="Type">
                        ...
                        >
                            ...
                            <SelectItem key="transfer" textValue="Transfer">Transfer</SelectItem>
                        </Select>
                    </Field>
                </div>
                <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="For Who (Contact)">
                    ...
                    {form.contactId && (
                        <div className="flex items-center gap-2 mt-2">
                            ...
                            <label htmlFor="isDebt" className="text-sm text-neutral-500">Record as Debt</label>
                        </div>
                    )}
                </Field>
                <Field label="Description">
                    <Textarea placeholder="Optional note..." value={form.description || ''} onChange={e => handleFormChange('description', e.target.value)} rows={2} />
                </Field>
                
                <div className="flex items-center gap-2">
                    <input type="checkbox" id="split" checked={isSplit} onChange={e => setIsSplit(e.target.checked)} className="rounded border-neutral-300 dark:border-neutral-700 bg-transparent text-primary focus:ring-primary" />
                    <label htmlFor="split" className="text-sm text-neutral-500">Split transaction</label>
                </div>
                        ...
                        {splits.map((s, i) => (
                            <div key={i} className="flex gap-2 items-center">
                                ...
                                <AmountInput 
                                    ...
                                    placeholder="0.00" 
                                    value={s.amount} 
                                    onChange={e => { const n = [...splits]; n[i].amount = e.target.value; setSplits(n); }} 
                                />
                                <Button isIconOnly size="sm" variant="light" color="danger" onClick={() => setSplits(splits.filter((_, j) => j !== i))}>✕</Button>
                            </div>
                        ))}
                        <Button type="button" variant="light" size="sm" color="primary" onClick={() => setSplits([...splits, { categoryId: '', amount: '' }])}>+ Add split</Button>
                    </div>
                )}
                ...
                <div className="flex gap-2 justify-end pt-4 border-t border-neutral-100 dark:border-neutral-800">
                    ...
                    <Button color="primary" type="submit" isLoading={create.isPending || update.isPending}>
                        {isEdit ? 'Save Changes' : (isDuplicate ? 'Duplicate Transaction' : 'Add Transaction')}
                    </Button>
                </div>
            </form>
        </Modal>
...
export function Transactions() {
    ...
    const [filters, setFilters] = useState({ type: 'all', search: '', walletId: 'all', categoryId: 'all', contactId: 'all', sortDate: 'newest' });
    ...
    const handleDelete = async () => {
        try { await remove.mutateAsync(confirmDel); toast('Deleted', 'success'); } catch { toast('Error deleting', 'error'); }
        setConfirmDel(null);
    };

    const handleExport = () => { window.open('/api/export/transactions/csv', '_blank'); };
    ...
    const renderCell = React.useCallback((tx, columnKey) => {
        ...
        switch (columnKey) {
            case "date":
                return (
                    <div className="flex flex-col">
                        <p className="text-bold text-sm text-neutral-900 dark:text-white">{format(parseISO(tx.date), 'MMM d, yyyy')}</p>
                        <p className="text-bold text-xs text-neutral-500 capitalize">{format(parseISO(tx.date), 'EEEE')}</p>
                    </div>
                );
            case "wallet":
                return (
                    <div className="flex flex-col">
                        <p className="text-bold text-sm text-neutral-900 dark:text-white">{tx.wallet?.name || '—'}</p>
                        {tx.type === 'transfer' && tx.to_wallet && (
                            <p className="text-bold text-xs text-neutral-500">→ {tx.to_wallet.name}</p>
                        )}
                    </div>
                );
            case "amount":
            ...
            case "category":
                return (
                    <Chip variant="flat" size="sm" style={{ backgroundColor: tx.category?.color + '20', color: tx.category?.color }}>
            ...
            case "contact":
                return (
                    <div className="flex items-center gap-2">
                        ...
                        {tx.is_debt && <Chip size="xs" color="primary" variant="solid" className="h-4 text-[9px]">DEBT</Chip>}
                    </div>
                );
            case "description":
                return (
                    <p className="text-sm text-neutral-500 max-w-[200px] truncate" title={tx.description}>
                        {tx.description || '—'}
                    </p>
                );
            case "actions":
                return (
                    <div className="relative flex items-center gap-1">
                        <Tooltip content="Edit">
                            <Button isIconOnly size="sm" variant="light" onClick={() => setModal({ intent: 'edit', data: tx })}>
                                <PencilIcon className="h-4 w-4 text-neutral-400" />
                            </Button>
                        </Tooltip>
                        <Tooltip content="Duplicate">
                            <Button isIconOnly size="sm" variant="light" onClick={() => setModal({ intent: 'duplicate', data: tx })}>
                                <DocumentDuplicateIcon className="h-4 w-4 text-neutral-400" />
                            </Button>
                        </Tooltip>
                        <Tooltip color="danger" content="Delete">
                            <Button isIconOnly size="sm" variant="light" color="danger" onClick={() => setConfirmDel(tx.id)}>
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 max-w-[1400px] mx-auto animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-neutral-900 dark:text-white">Transactions</h1>
                    <p className="text-neutral-500">Manage and track your cash flow</p>
                </div>
                <div className="flex gap-2">
                    <Button variant="bordered" startContent={<ArrowDownTrayIcon className="h-4 w-4" />} onClick={handleExport}>
                        Export CSV
                    </Button>
                    <Button color="primary" startContent={<PlusIcon className="h-4 w-4" />} onClick={() => setModal('new')}>
                        Add Transaction
                    </Button>
                </div>
            </div>
            ...
            <div className="glass-card backdrop-blur-md rounded-3xl overflow-hidden shadow-xl">
                ...
                >
                    <TableHeader columns={columns}>
                        {(column) => (
                            <TableColumn key={column.uid} className="bg-neutral-100/50 dark:bg-neutral-800/50 text-muted-foreground font-bold uppercase py-4">
                                {column.name}
                            </TableColumn>
                        )}
                    </TableHeader>
                    <TableBody 
                        ...
                        emptyContent={<EmptyState icon={ArrowsRightLeftIcon} title="No transactions found" description="Adjust your filters or add a new transaction." />}
                    >
                        {(item) => (
                            <TableRow key={item.id} className="border-b border-neutral-200 dark:border-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-800/50 transition-colors">
            ...
            {/* Modals */}
            {modal === 'new' && <TransactionModal open onClose={() => setModal(null)} />}
            {modal && modal !== 'new' && <TransactionModal open onClose={() => setModal(null)} transaction={modal} />}
            <ConfirmModal open={!!confirmDel} title="Delete Transaction" description="This will permanently delete the transaction and reverse the wallet balance." onConfirm={handleDelete} onCancel={() => setConfirmDel(null)} />
        </div>
    );
}
